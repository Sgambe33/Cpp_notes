...
LSPClient::LSPClient(QString path, QStringList args)
{
    ...
    connect(clientProcess, SIGNAL(errorOccurred(QProcess::ProcessError)), this, SLOT(onClientError(QProcess::ProcessError)));
    ...
    connect(clientProcess, SIGNAL(finished(int, QProcess::ExitStatus)), this, SLOT(onClientFinished(int, QProcess::ExitStatus)));
...
void LSPClient::onClientReadyReadStdout()
{
    ...
    if (obj.contains("id"))
    {
        ...
        {
            emit onResponse(obj["id"].toString(), obj["result"].toObject());
        }
...
{
    qDebug() << "[CLIENT TERMINATED] "<<status;
...
void LSPClient::didChange(DocumentUri uri, std::vector<TextDocumentContentChangeEvent> &changes, option<bool> wantDiagnostics)
{
