...
Mainwindow::Mainwindow(QWidget *parent) : QMainWindow(parent)
{
    ...
    lsp = new LSPClient("tinymist-win32-x64.exe", {});
    setConnections();
    file.open();

    file.rename("C:/Users/Casa/Desktop/sol.typ");
...
//    file.write(code->toPlainText().toLocal8Bit());
    ...
    lsp->didChange("file://" + file.fileName().toStdString(), changes, true);

    QChar lastChar = ev.text.length()==0 ? QChar() : ev.text.back();
    QStringList triggers = {"#","(","<",",",".",":","/","\"","@"};
    // Check for completion triggers (e.g., '.', '::')
    if (triggers.contains(lastChar)) {
        requestCompletions();
    }
...
void Mainwindow::requestCompletions()
{
    auto cursor = code->textCursor();
    int line = cursor.blockNumber();          // 0-based line number
    int character = cursor.positionInBlock(); // 0-based column

    Position pos;
    pos.character = character;
    pos.line = line;

    lsp->completion("file://" + file.fileName().toStdString(), pos);
    qDebug() << "Requested completions at line" << line << "character" << character;
}
...
void Mainwindow::OnResponse(QString id, QJsonObject response)
{
    info->appendPlainText("Response[" + id + "]: " + jsonObjectToString(response));

    if (id == "initialize") {
        lsp->initialized();
        lspReady = true;
        info->appendPlainText("LSP Initialized, now opening file...");
        lsp->didOpen("file://" + file.fileName().toStdString(), "", "typ");
    }
}
